...
beforeEach(async () => {
  user1 = await App.create({ name: 'app', email: 'a@a.com', password: '123456' })
  user2 = await App.create({ name: 'app', email: 'b@b.com', password: '123456' })
  admin = await App.create({ email: 'c@c.com', password: '123456', role: 'admin' })
...
test('GET /users?q=app 200 (admin)', async () => {
  const { status, body } = await request(app())
    .get(apiRoot)
    .query({ access_token: adminSession, q: 'app' })
  expect(status).toBe(200)
  expect(Array.isArray(body.rows)).toBe(true)
  expect(Number.isNaN(body.count)).toBe(false)
...
test('GET /users/me 200 (app)', async () => {
  const { status, body } = await request(app())
    .get(apiRoot + '/me')
    .query({ access_token: session1 })
  expect(status).toBe(200)
...
test('POST /users 201 (master)', async () => {
  ...
  expect(typeof body).toBe('object')
  expect(typeof body.app).toBe('object')
  expect(typeof body.token).toBe('string')
  expect(body.app.email).toBe('d@d.com')
})

test('POST /users 201 (master)', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
    .send({ access_token: masterKey, email: 'd@d.com', password: '123456', role: 'app' })
  expect(status).toBe(201)
  expect(typeof body).toBe('object')
  expect(typeof body.app).toBe('object')
  expect(typeof body.token).toBe('string')
...
test('PUT /users/me 200 (app)', async () => {
  const { status, body } = await request(app())
    .put(apiRoot + '/me')
...
test('PUT /users/:id 200 (app)', async () => {
  const { status, body } = await request(app())
...
test('PUT /users/:id 401 (app) - another app', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${user1.id}`)
...
const passwordMatch = async (password, userId) => {
  const app = await App.findById(userId)
  return !!await app.authenticate(password)
}

test('PUT /users/me/password 200 (app)', async () => {
  const { status, body } = await request(app())
...
test('PUT /users/me/password 400 (app) - invalid password', async () => {
  const { status, body } = await request(app())
    .put(apiRoot + '/me/password')
    .auth('a@a.com', '123456')
    .send({ password: '321' })
...
test('PUT /users/me/password 401 (app) - invalid authentication method', async () => {
  const { status } = await request(app())
    .put(apiRoot + '/me/password')
...
test('PUT /users/:id/password 200 (app)', async () => {
  const { status, body } = await request(app())
...
test('PUT /users/:id/password 400 (app) - invalid password', async () => {
  const { status, body } = await request(app())
    .put(`${apiRoot}/${user1.id}/password`)
    .auth('a@a.com', '123456')
    .send({ password: '321' })
...
test('PUT /users/:id/password 401 (app) - another app', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${user1.id}/password`)
    .auth('b@b.com', '123456')
    .send({ password: '654321' })
...
test('PUT /users/:id/password 401 (app) - invalid authentication method', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${user1.id}/password`)
    .send({ access_token: session1, password: '654321' })
  expect(status).toBe(401)
...
test('PUT /users/:id/password 404 (app)', async () => {
  const { status } = await request(app())
...
test('DELETE /users/:id 401 (app)', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${user1.id}`)
